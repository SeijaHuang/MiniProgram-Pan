...
- **Layer 3 - UI 交互层**
    - `welcome-page__actions`，承载主 CTA 按钮。
- **Layer 4 - 弹窗层**
    - 昵称设置弹窗（`nickname-modal`），`z-index: 1000`，固定定位覆盖全屏。
...
- **实现原则**（与 PRD 对齐）：
    ...
    - **时序配置**：使用 `TIMING` 常量统一管理各阶段延迟和时长
        - 总时长：约 1.8 秒（1300ms CTA 入场 + 500ms 动画时长）
        - 各阶段延迟：标题 100ms → 头像 400ms → tagline 900ms → CTA 1300ms
...
- **我要申冤！**：`handleStartJudge` ✅ **已实现**
    - 逻辑：读取 `globalData.userInfo.nickName`，有值则直接跳转等待页；无值则调用 `openNicknameModal()` 弹出昵称设置弹窗
    - 按钮样式：红色渐变背景、粗黑描边（8rpx，底部 12rpx）、底部厚度感阴影、大圆角（24rpx）
    - 文字样式：白色、40rpx、字重 700、上下 padding 50rpx

#### 4.3 昵称设置弹窗（nickname-modal）✅ **已实现**

首次进入且无缓存昵称时，点击「我要申冤！」会触发底部弹窗。
...
│    堂下何人，报上名来！   │  ← 标题
│                         │
│  [ 点击从微信昵称中选择 ] │  ← type="nickname" 输入框 + x/12 字数
│                         │
│     [ 击鼓申冤！]        │  ← 主按钮（昵称为空时禁用）
│       稍后再说           │  ← 次级按钮（跳过，使用「申冤人」进入）
└─────────────────────────┘
```
...
| 函数                       | 说明                                                              |
| -------------------------- | ----------------------------------------------------------------- |
| `openNicknameModal()`      | 打开弹窗并播放上滑动画                                            |
| `onNicknameInput(e)`       | 输入变化时更新 `nicknameInput` / `nicknameCharCount` / 按钮禁用态 |
| `onNicknameConfirm()`      | 保存昵称（`nicknameService.saveNickName`）并跳转等待页            |
| `onNicknameSkip()`         | 临时使用「申冤人」进入，不写入 Storage                            |
| `onNicknameModalMaskTap()` | 点击遮罩关闭弹窗                                                  |

**验证规则**（`nicknameService.validate`）：非空 + 非纯空格 + 长度 ≤ 12。

**Storage Key**：`userNickName`（持久化）。

---

### 5. 状态与后续规划

#### 5.1 当前实现状态（2026-02-21）

- ✅ **已完成**：
    - 欢迎页面基础结构搭建（三大区域：Hero + CTA + 弹窗层）
    - 135° 渐变背景（红→橙→黄）
    ...
    - **完整入场动画序列**：
        - 4 个阶段的分阶段入场（标题、头像、tagline、CTA）
        - 使用 `wx.createAnimation` 实现，时序统一管理
        - 入场动画只播放一次，后续页面显示时恢复呼吸动画
    - 主 CTA 按钮「我要申冤！」：
        - 红色渐变背景 + 粗黑描边 + 底部厚度感
        - 弹性缩放入场动画
        - 有昵称 → 直接跳转等待页；无昵称 → 弹出昵称设置弹窗
    - **昵称设置弹窗**：
        - 底部半屏弹出，上滑动画，遮罩可关闭
        - `type="nickname"` 输入框（支持直接填入微信昵称）
        - 实时字数统计（最多 12 字），昵称为空时禁用主按钮
        - 「击鼓申冤！」保存昵称 + 跳转；「稍后再说」临时使用「申冤人」
    - **用户身份初始化**（`onLoad`）：
        - 读取 / 生成 `userId`（持久化到 Storage）
        - 读取 `userNickName` 并写入 `globalData.userInfo.nickName`
- ⏳ **待实现 / 待完善**：
    - 其他装饰元素的动画（如云朵、几何图形等）
