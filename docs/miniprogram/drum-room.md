...
| 可点击 | 正常状态（红色渐变） | 10秒抢麦期间可点击    |
| 已结束 | 禁用状态（灰色）     | 10秒倒计时结束后锁定  |
...
| `drum_countdown_end` | 抢麦倒计时结束        | 10秒倒计时归零 |
| `drum_winner`        | 抢麦获胜              | 结果判定为胜者 |
| `drum_loser`         | 抢麦失败              | 结果判定为败者 |
| `drum_timeout`       | 抢麦超时（双方0点击） | 10秒内无点击   |
